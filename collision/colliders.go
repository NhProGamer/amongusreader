...
// colliderPaths holds the raw SVG path strings for each map, ported directly
// from ColliderMap.ts.  The paths use SVG coordinate space where
//
//	svgX = worldX + 40
//	svgY = 40  - worldY
var colliderPaths = map[types.MapType][]string{
...
// ColliderSegments returns the pre-parsed wall segments for the given map type.
// The slice is lazily computed and cached on first call.
func ColliderSegments(mapType types.MapType) [][]Segment {
	// normalise April Fool's variant
	if mapType == types.MapTypeTheSkeldApril {
		mapType = types.MapTypeTheSkeld
	}
	paths, ok := colliderPaths[mapType]
	if !ok {
		return nil
	}
	result := make([][]Segment, 0, len(paths))
	for _, p := range paths {
		segs, err := ParseSVGPath(p)
...
// RawColliderPaths returns the raw SVG path strings for the given map type.
// Returns nil for unknown / unsupported maps.
func RawColliderPaths(mapType types.MapType) []string {
	if mapType == types.MapTypeTheSkeldApril {
		mapType = types.MapTypeTheSkeld
	}
	return colliderPaths[mapType]
}
